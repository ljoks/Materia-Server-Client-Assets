...
app.controller('SupportUserController', function (Please, $scope, $window, SupportSrv, UserServ) {
	let lastSearch = ''
	...
	const _searchFor = (nameOrFragment) => {
		...
		SupportSrv.searchUsers(nameOrFragment).then((result) => {
	...
	const searchMatchClick = (user) => {
		SupportSrv.lookupUser(user.id).then((data) => {
	...
	const save = () => {
		...
		SupportSrv.saveUser(update).then((response) => {
			let errors = []
